...
export async function POST(request: Request) {
  try {
    ...
    // Cek kuota distribusi per periode
    ...
    await prisma.statusPesertaLokaID.upsert({
      ...
      update: { status: "sudah_terima" },
      create: { pesertaId: peserta.id, programId: program.id, status: "sudah_terima", periode },
    });
...
function getPeriodeStart(reset: string): Date {
  const now = new Date();
  if (reset === "harian") return new Date(now.getFullYear(), now.getMonth(), now.getDate());
  if (reset === "mingguan") {
