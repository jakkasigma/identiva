...
const schema = z.object({
  ...
  keterangan: z.string().optional(), // nama item yang dipinjam
});
...
export async function POST(request: Request) {
  try {
    ...
    const statusAktif = await prisma.statusPesertaLokaID.findUnique({
      where: { pesertaId_programId_periode: { pesertaId: peserta.id, programId: program.id, periode } },
    });
    if (statusAktif && ["mengajukan", "meminjam"].includes(statusAktif.status)) {
      return Response.json({ status: "invalid", alasan: "sudah_ada_pengajuan" });
    }

    await prisma.$transaction([
      prisma.aktivitasLokaID.create({
        data: { pesertaId: peserta.id, programId: program.id, mitraId: mitra.id, cabangId: cabang.id, jenis: "pengajuan", keterangan: body.keterangan ?? null },
      }),
      prisma.statusPesertaLokaID.upsert({
        where: { pesertaId_programId_periode: { pesertaId: peserta.id, programId: program.id, periode } },
        update: { status: "mengajukan" },
        create: { pesertaId: peserta.id, programId: program.id, status: "mengajukan", periode },
      }),
    ]);
